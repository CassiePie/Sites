...
<div class="bg-white p-4">
    ...
    <form action="{{ route('discount.set') }}" method="post" class="mt-2 flex gap-4">
        @csrf
        <input placeholder="Code" name="code" class="uppercase bg-white appearance-none px-4 py-2 border border-gray-500 {{ session('discount') ? 'bg-gray-100' : '' }}" {{ session('discount') ? 'disabled' : '' }}">
        <button type="submit" class="text-xl" {{ session('discount') ? 'disabled' : '' }}>
            <i class="fa-solid fa-circle-check"></i>
        </button>
    </form>
    @if(session('discount-error'))
