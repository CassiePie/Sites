    ...
    <div class="container px-8 mx-auto flex justify-between items-center">
        ...
        <div class="flex gap-4 text-xl items-center">
            ...
            <a href="{{ route('cart') }}" class="bg-gray-200 px-4 py-1 rounded-full">
                ...
                <span>{{ isset($products) ? $products->count() : 0 }} {{ Str::plural('item', isset($products) ? $products->count() : 0) }}</span>
                {{-- <span>{{ $products->count() }} {{ Str::plural('item', isset($products) ? $products->count() : 0) }}</span> --}}
            </a>
        </div>
    </div>
</header>
