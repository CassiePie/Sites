    ...
    <div class="max-w-2xl mx-auto">
        ...
        <form action="{{ route('orders.store') }}" method="post" class="flex flex-col gap-4 pt-4">
            ...
            <div class="flex flex-col">
                ...
                <input name="voornaam" value="{{ old('voornaam') }}" type="text" class="bg-white border border-gray-500 px-4 py-2">
                @error('voornaam') <p class="text-red-500">{{ $message }}</p> @enderror
            </div>
            <div class="flex flex-col">
                <label class="text-gray-500" for="achternaam">Achternaam: *</label>
                <input name="achternaam" value="{{ old('achternaam') }}" type="text" class="bg-white border border-gray-500 px-4 py-2">
                @error('achternaam') <p class="text-red-500">{{ $message }}</p> @enderror
            </div>
            <div class="grid grid-cols-5 gap-4">
                <div class="flex flex-col col-span-4">
                    <label class="text-gray-500" for="straat">Straat: *</label>
                    <input name="straat" value="{{ old('straat') }}" type="text" class="bg-white border border-gray-500 px-4 py-2">
                    @error('straat') <p class="text-red-500">{{ $message }}</p> @enderror
                </div>
                <div class="flex flex-col">
                    <label class="text-gray-500" for="huisnummer">Huisnummer: *</label>
                    <input name="huisnummer" value="{{ old('huisnummer') }}" type="text" class="bg-white border border-gray-500 px-4 py-2">
                    @error('huisnummer') <p class="text-red-500">{{ $message }}</p> @enderror
                </div>
            </div>
            <div class="grid grid-cols-5 gap-4">
                <div class="flex flex-col">
                    <label class="text-gray-500" for="postcode">Postcode: *</label>
                    <input name="postcode" value="{{ old('postcode') }}" type="text" class="bg-white border border-gray-500 px-4 py-2">
                    @error('postcode') <p class="text-red-500">{{ $message }}</p> @enderror
                </div>
                <div class="flex flex-col col-span-4">
                    <label class="text-gray-500" for="woonplaats">Woonplaats: *</label>
                    <input name="woonplaats" value="{{ old('woonplaats') }}" type="text" class="bg-white border border-gray-500 px-4 py-2">
                    @error('woonplaats') <p class="text-red-500">{{ $message }}</p> @enderror
                </div>
            </div>
            <div>
