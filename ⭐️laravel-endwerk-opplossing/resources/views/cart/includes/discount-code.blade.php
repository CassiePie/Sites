...
<div class="bg-white p-4 @if(session('discount')) opacity-50 @endif">
    ...
    <form action="{{ route('discount.set') }}" method="post" class="mt-2 flex gap-4">
        @csrf
        <input @if(session('discount')) disabled @endif name="code" value="{{ old('code') }}" placeholder="Code" class="uppercase bg-white appearance-none px-4 py-2 border border-gray-500">
